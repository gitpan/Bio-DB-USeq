...
Currently, C<Bio::DB::USeq> can only parse files that do not contain 
text strings. This is a current technical limitation that should 
be overcome.

=head2 Requirements

The adaptor is a Perl-only implementation. It only requires the 
C<Archive::Zip> module for opening and reading the useq file archive. 
...
Return the C<Archive::Zip> object representing the useq file archive.
...
iterator object is returned. See L<get_seq_stream> for details.
...
C<Bio::DB::Sam> adaptor for use with C<Bio::Graphics> and GBrowse. 
Note that only scores are returned, not a true depth coverage 
in the sense of the C<Bio::DB::Sam> coverage types. 
...
C<Bio::DB::BigWig> adaptor and may be used interchangeably. They 
are compatible with the C<Bio::Graphics> modules.
...
C<Bio::Graphics> and GBrowse. They support the wiggle() and coverage() 
...
generated by the C<Bio::DB::BigWig> database adaptor. As such, they are 
compatible with C<Bio::Graphics> and GBrowse. 
...
from the C<Bio::DB::BigWig> adaptor and may be used interchangeably. 
...
The USeq adaptor has support for C<Bio::Graphics> and GBrowse. 
...
wiggle feature type is faster than the summary feature type. Very 
large USeq files (half a gigabyte or larger) may see better 
performance when converted to a BigWig file, especially when viewing 
large intervals.
=cut
...
sub _parse_members {
	...
	foreach my $member ($self->{'zip'}->memberNames) {
		...
		# The perl unpack function cannot handle Java binary text strings
		if ($extension =~ /t/) {
			carp " Cannot parse USeq archives containing text!\n";
			return;
		}
...
Interpret text fields from slices containing text. The text fields are 
written using Java's WriteUTF function, which is a modified UTF-8 format, 
wherein a short is first written to record the length of the subsequent 
UTF-8 string. This is going to require a complicated unpack that loops 
through the binary string one observation at a time. Ugh.
